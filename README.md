...
已支持 `sentence`、`10k`、`200k` 三档，各自排名、不合成总分；`100k` 与 `1k` 尚未上线，不在榜单中。
...
- **评测档位**：一次运行只选一档，未指定时为 `200k`。`sentence` 的用户内容为单句中文短文指令（cl100k_base 计数 61），不附加 Django 切片与分隔标记；`10k` 的任务说明与 `prompts/task_200k.md` 全文一致，附加 `fixtures/django_10k.txt` 全文（cl100k 10000，不截断）；`200k` 保持现有任务说明与 `fixtures/django_200k.txt`（cl100k 200000）。
...
- **分档榜 `data/latest_{200k,10k,sentence}.json`**：同一份 `data/results.jsonl` 聚合成三份扁平数组（各档一份），行内 `scenario` 自描述，档内按端到端 TPS 降序，互不混排，不合成总分。
...
# 一句话档 / 10K 档（一次运行只选一档）
python3 scripts/run_bench.py --scenario sentence
python3 scripts/run_bench.py --scenario 10k
...
由 `data/results.jsonl` 重新生成分档榜（`data/latest_200k.json` 等）：
