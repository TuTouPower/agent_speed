...
def test_opencode_metrics_server_window():
    ...
    ttft, decode_window, source, in_toks, out_toks = parse_opencode_metrics(lines)
...
def test_grok_metrics_last_content_minus_ttft():
    ...
    ttft, decode_window, source, in_toks, out_toks = parse_grok_metrics(lines)
    assert ttft == 1.5
...
def test_kimi_metrics_decode_ms():
    ...
    ttft, decode_window, source, in_toks, out_toks = parse_kimi_metrics(lines, wire_info)
    assert ttft == 1.8
...
def test_codex_metrics_empty_window():
    ...
    ttft, decode_window, source, in_toks, out_toks = parse_codex_metrics(lines)
    assert ttft == 2.1
