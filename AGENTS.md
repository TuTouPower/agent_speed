...
|`data/latest_200k.json`|200k 档公开上站聚合表（覆盖写）|公开跟踪的上站数据文件，按端到端 TPS 降序|
|`data/latest_10k.json`|10k 档公开上站聚合表（覆盖写）|由 `report.py` 写出；可暂无文件|
|`data/latest_sentence.json`|sentence 档公开上站聚合表（覆盖写）|由 `report.py` 写出；可暂无文件|
|`data/models.json`|本仓权威模型身份表（覆盖写；`id` + 可选定价仓 `pricing_aliases`）|由模型表维护 task / 后续维护更新；测速流水线只读|
|`data/latest_pricing.json`|对齐后的单价表（覆盖写；按 `real_usd_per_mtok` 可排序）|由 `scripts/update_pricing.py` 写出；测速流水线不读|
|`data/unmatched_pricing.json`|定价仓采用表未对齐本仓模型表的清单（覆盖写）|由 `scripts/update_pricing.py` 写出；驱动补录模型表别名|
...
|`artifacts/` `.scratch/`|产物与一次性草稿|运行与草稿；debug 复现和临时实验只写`.scratch/`（已 gitignore）；需保留的 spike 验证材料写 `docs/spikes/{sid}_{slug}/code/`。`data/` 下仅跟踪 `results.jsonl` / `latest_200k.json` / `latest_10k.json` / `latest_sentence.json` / `models.json` / `latest_pricing.json` / `unmatched_pricing.json` / `.gitkeep`，其余 runtime 忽略|
