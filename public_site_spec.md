...
- 网站：私有仓库 `站点仓库` 的 `web`。静态页。不接登录、数据库、计费，不引用该仓库的 auth、db、billing。网站不实现中位数和过滤，只渲染已经算好的 `latest.json`。
...
- 把网站放在 `个人域名`，或接上 `站点仓库` 的登录、数据库、计费。
...
- 公开仓库不含 `内部项目` 切片、旧跑分、模型正文、`events/`、密钥和本机绝对路径。
...
- 只上传静态文件时，本机不执行测试和打包，`站点仓库` 的 GitHub Actions 不触发。
