...
def resolve_web_dir(explicit: str | None) -> Path | None:
    ...
    env = os.environ.get("AGENT_SPEED_WEB_DIR", "").strip()
    if env:
        candidates.append(Path(env).expanduser().resolve())
    candidates.extend(
        [
            (REPO_ROOT.parent / "great_websites" / "systems" / "agent_speed" / "web").resolve(),
            (REPO_ROOT / "web").resolve(),
        ]
    )
    for p in candidates:
...
def serve_and_shot(
    ...
    with tempfile.TemporaryDirectory(prefix="agent-speed-board-") as tmp:
        tmp_dir = Path(tmp)
        # copy board assets
