...
- `src/`：核心测速与分析工具链
    - `agent_speed/`：评测驱动核心包，提供数据模型（models）、档位输入选择（scenarios：`sentence` / `10k` / `200k` 一次一档，未指定为 `200k`）、各 harness（opencode/grok-build/codex/kimi-code/antigravity）适配器、指标计算（metrics）、双层受控并发调度器（scheduler）以及报告生成模块（report：三份榜分文件，不合成总分）。
- `report.py`：公开报告生成脚本，读 `data/results.jsonl` 按最新 batch、有效次数与账单输入过滤规则计算中位数并覆盖写三份榜：`data/latest.json`（只含 `200k`）、`data/latest_10k.json`（只含 `10k`）、`data/latest_sentence.json`（只含 `sentence`）。
- `scripts/`：项目构建与运行脚本。
...
- `data/latest.json`：公开展示站聚合汇总表（覆盖写，只含 `200k`）。
- `data/latest_10k.json`：10K 档聚合汇总表（覆盖写，只含 `10k`，无上站行时为 `[]`）。
- `data/latest_sentence.json`：一句话档聚合汇总表（覆盖写，只含 `sentence`，无上站行时为 `[]`）。
- `tests/`：自动化测试套件。
- `docs/`：项目规范与知识库。
