...
[scripts/run_bench.py] ──(追加写入)──► data/results.jsonl (明细流水账)
                                       ...
                                       (提取各网格最新 batch_id)
                                              ▼
[report.py]            ──(覆盖重写)──► data/latest_{200k,10k,sentence}.json
```
...
1. **`data/results.jsonl`（只追加不修改）**：
    ...
    - 绝不手工删除或修改历史行。
2. **`data/latest_*.json`（分档覆盖）**：
    - `report.py` 扫描全部流水账，对每个网格单元**仅锁定最后出现的 `batch_id`**；
    - 只要某个模型重跑产生了新 batch，重新运行 `report.py` 时，该模型在 `data/latest_200k.json` 中的指标就会自动被新批次中位数覆盖，而未重跑的模型继续沿用其各自的上一次最新批次，互不干扰。
...
- `data/latest_200k.json` 中该模型的中位数指标刷新为最新批次数据，其他模型数据完全保留，整体表格按端到端 TPS 自动重排。
...
新模型只要满足上站门槛（有效次数 ≥ 2、账单输入 token ≥ 100,000），即自动插入 `data/latest_200k.json`。

______________________________________________________________________

## 4. 场景三：三档独立评测（sentence / 10k / 200k）

一次运行只选一档，未指定时为 `200k`。同一份 `data/results.jsonl` 聚合出分档榜 `data/latest_{200k,10k,sentence}.json`（扁平行内自描述，互不混排，不合成总分）。
...
使用 `scripts/check_coverage.py` 进行只读检测，无需修改任何数据文件，快速查看全矩阵各档覆盖状态：

```bash
# 全档位检测（sentence / 10k / 200k）
uv run python scripts/check_coverage.py

# 仅检测指定档位
uv run python scripts/check_coverage.py --scenarios sentence,10k
```

### 输出状态标识

- **`ok `**：已达到上榜门槛（有效成功 ≥ 2 次，账单输入中位数合格）并已上榜。
- **`MISS`**：该网格在流水账中尚无有效调用记录。
- **`thin`**：虽有成功记录但有效次数不足 2 次，无法上榜。
- **`GATE`**：调用成功但账单输入中位数低于对应档位输入门槛（如长上下文被截断），即使补跑也无法直接上榜。
- 脚本会自动针对 `MISS` 与 `thin` 格子输出直接可用的补跑命令。
...
2. **测试结果未进入榜单的排查检查单**：
    ...
    - **账单输入被截断**：事件流返回的 `in_tokens` 中位数是否低于该记录 `cl100k_tokens` 的一半（`200k` 为 100,000，`10k` 为 5,000，`sentence` 为 30.5，等于一半上站）？若低于门槛，契约判定为上下文严重丢失，拒绝上站。注意在对应分档榜内排查。
